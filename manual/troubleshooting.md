...
## blobs/jobs still show up in tools/list after setting HOUTINI_LM_BLOBS=0 / HOUTINI_LM_JOBS=0

**Almost certainly your client's own tool-catalog cache, not the server.** Set directly against `dist/index.js` and queried fresh over the stdio transport, both kill switches remove the tool and its parameters from the schema immediately - `blobs` and `context_blob_id` disappear together, `jobs` and `async` disappear together. If a client still shows them after a redeploy with the variable flipped, the most likely cause is an MCP aggregator sitting in front of the server (a portal that fans out to several backend MCP servers) caching its combined tool catalog per session. Reconnecting your own MCP connection isn't always enough in that case - the aggregator's own session may need to be reconnected (or a fresh one started) before it re-queries the backend and picks up the change.
