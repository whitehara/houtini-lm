...
### Added
- **`blobs` tool and `custom_prompt`'s `context_blob_id`** — upload a prompt too large for a single MCP `tools/call` body in ordered chunks (`create` → one or more `append` → `seal`), then use the sealed blob's id as `custom_prompt`'s `context_blob_id` instead of pasting it inline — the blob's full body is substituted for `context` verbatim, and pairing it with `async: true` is the intended way to submit input too large or too slow to process synchronously. 50,000 characters per chunk is the recommended size. A new `blobs` tool manages them (`create`/`append`/`seal`/`list`/`delete` — `list` metadata only, never body content, strictly scoped to blobs you own). `context_blob_id` cannot be combined with `context` or with `start_conversation`/`conversation_id`. Entirely opt-in: leave `context_blob_id` unset and `custom_prompt` behaves exactly as before. Blobs are held in memory only — lost on restart/redeploy, not shared across Swarm replicas, share `conversations`'/`jobs`' ownership boundary (`HOUTINI_LM_CONVERSATION_OWNER_HEADER`), and (unlike jobs) are swept by their idle TTL even while still `open` (mid-upload) — governed by five new env vars: `HOUTINI_LM_BLOBS` (default `1`; set to `0`/`false`/`no`/`off` to disable the feature entirely — also a production kill switch — including the `blobs` tool and `context_blob_id` disappearing from the schema), `HOUTINI_LM_BLOB_TTL_MIN` (default `30`), `HOUTINI_LM_BLOB_MAX` (default `20`), `HOUTINI_LM_BLOB_MAX_CHARS` (default `2000000`), and `HOUTINI_LM_BLOB_MAX_TOTAL_CHARS` (default `8000000`). See [Large payloads (blobs)](./README.md#large-payloads-blobs).
- **`jobs get` output-side chunking** — `jobs get` now accepts `offset`/`limit` to page through a large completed result instead of getting it all at once; a result past the new `HOUTINI_LM_JOB_RESULT_INLINE_MAX_CHARS` (default `50000`) is automatically chunked this way even without asking, with the response carrying a `Next` continuation footer telling the caller exactly what to pass on the following call (copy it verbatim rather than computing the next offset). `jobs list` also now reports each job's result size in characters. A `failed` job's `error` is always returned in full, never chunked. Set `HOUTINI_LM_JOB_RESULT_INLINE_MAX_CHARS=0` to disable automatic chunking only — explicit `offset`/`limit` requests still work and still chunk. See [Async jobs](./README.md#async-jobs) and [jobs](./README.md#jobs).
- **Async jobs** — `async` (boolean) on `custom_prompt` and `code_task_files` submits the call as a background job instead of waiting for the answer inline: the response is a job id, returned before inference starts (after any file reads/validation for `code_task_files`, which still happen synchronously). Retrieve the result with a new `jobs` tool (`list` for metadata only, `get` — with optional `wait_ms` polling, capped at 30,000ms — for status and, once `completed`/`failed`, the result or error, `delete` to forget a record without cancelling in-flight inference). Exists because MCP clients time out a `tools/call` well before a reasoning-heavy model or a large `code_task_files` input finishes, even though the server's own soft timeout is five minutes. Entirely opt-in: leave `async` unset and both tools behave exactly as before. `async` cannot be combined with `start_conversation`/`conversation_id`. Jobs are held in memory only — lost on restart/redeploy, not shared across Swarm replicas, and share `conversations`' ownership boundary (`HOUTINI_LM_CONVERSATION_OWNER_HEADER`) — governed by seven new env vars: `HOUTINI_LM_JOBS` (default `1`; set to `0`/`false`/`no`/`off` to disable the feature entirely — also a production kill switch — including the `jobs` tool disappearing from `tools/list`), `HOUTINI_LM_JOB_TTL_MIN` (default `60`), `HOUTINI_LM_JOB_MAX` (default `50`), `HOUTINI_LM_JOB_ACTIVE_MAX_PER_OWNER` (default `2`), `HOUTINI_LM_JOB_CONCURRENCY` (default `1`), `HOUTINI_LM_JOB_SOFT_TIMEOUT_MIN` (default `15`), and `HOUTINI_LM_JOB_MAX_RESULT_CHARS` (default `200000`). See [Async jobs](./README.md#async-jobs).
...
- **Server-side conversations** — `start_conversation` (boolean) and `conversation_id` (string) on `chat` and `custom_prompt` let the server remember a conversation across calls, so the caller sends only new input on each follow-up instead of resending the whole transcript; `custom_prompt`'s `context` is recorded into that history only the first time it's sent for a given conversation, so resending it is safe. A new `conversations` tool manages them (`list` for metadata only, `delete` one, `clear` all — strictly scoped to conversations you own). Entirely opt-in: leave both parameters unset and `chat`/`custom_prompt` behave exactly as before. Conversations are held in memory only — lost on restart/redeploy, not shared across Swarm replicas, and, when `HOUTINI_LM_CONVERSATION_OWNER_HEADER` is unset (the default — see below), also discarded immediately if the owning MCP session is `DELETE`d over HTTP — governed by five new env vars: `HOUTINI_LM_CONVERSATIONS` (default `1`; set to `0`/`false`/`no`/`off` to disable the feature entirely, including the `conversations` tool disappearing from `tools/list`), `HOUTINI_LM_CONVERSATION_TTL_MIN` (default `60`), `HOUTINI_LM_CONVERSATION_MAX` (default `50`), `HOUTINI_LM_CONVERSATION_MAX_TURNS` (default `40`), and `HOUTINI_LM_CONVERSATION_MAX_CHARS` (default `48000`). See [Server-side conversations](./README.md#server-side-conversations).
- **`HOUTINI_LM_CONVERSATION_OWNER_HEADER`** (string, default unset) — names an HTTP request header carrying an authenticated caller identity; when set, server-side conversations are scoped to that identity instead of the MCP session, so the same caller can continue a conversation across sessions/connections (e.g. behind a gateway that reconnects on every call). Fails closed: a missing, empty, oversized, or duplicated header value is rejected outright rather than falling back to the session-scoped behaviour. **Only set this if the front-end proxy replaces any client-supplied header of the same name on every request** — an appending proxy would let a client spoof another user's identity; this server's duplicate-value rejection is a second line of defense, not a substitute for a correctly configured proxy. See [Server-side conversations § Isolation boundary](./README.md#server-side-conversations).
